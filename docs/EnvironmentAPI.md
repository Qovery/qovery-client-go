...
[**ListEnvironmentServicesLinks**](EnvironmentAPI.md#ListEnvironmentServicesLinks) | **Get** /api/environment/{environmentId}/link | List environment services links
...
func main() {
	environmentId := "38400000-8cf0-11bd-b23e-10b96e4ef00d" // string | Environment ID
	dockerfileCheckRequest := *openapiclient.NewDockerfileCheckRequest(*openapiclient.NewApplicationGitRepositoryRequest("https://github.com/Qovery/simple-node-app", openapiclient.GitProviderEnum("BITBUCKET")), "DockerfilePath_example") // DockerfileCheckRequest |  (optional)
...
## ListEnvironmentServicesLinks

> LinkResponseList ListEnvironmentServicesLinks(ctx, environmentId).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.EnvironmentAPI.ListEnvironmentServicesLinks(context.Background(), environmentId).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `EnvironmentAPI.ListEnvironmentServicesLinks``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `ListEnvironmentServicesLinks`: LinkResponseList
	fmt.Fprintf(os.Stdout, "Response from `EnvironmentAPI.ListEnvironmentServicesLinks`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiListEnvironmentServicesLinksRequest struct via the builder pattern
