...
> DeleteDatabase(ctx, databaseId).SkipReconcile(skipReconcile).Execute()
...
func main() {
	...
	r, err := apiClient.DatabaseMainCallsAPI.DeleteDatabase(context.Background(), databaseId).SkipReconcile(skipReconcile).Execute()
	if err != nil {
