...
type ApiListLogicalDatabaseContainerRequest struct {
	ctx               context.Context
	ApiService        *LogicalDatabaseApiService
	logicalDatabaseId string
}

func (r ApiListLogicalDatabaseContainerRequest) Execute() (*ContainerResponseList, *http.Response, error) {
	return r.ApiService.ListLogicalDatabaseContainerExecute(r)
}

/*
ListLogicalDatabaseContainer List linked containers

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param logicalDatabaseId Logical Database ID
 @return ApiListLogicalDatabaseContainerRequest
*/
func (a *LogicalDatabaseApiService) ListLogicalDatabaseContainer(ctx context.Context, logicalDatabaseId string) ApiListLogicalDatabaseContainerRequest {
	return ApiListLogicalDatabaseContainerRequest{
		ApiService:        a,
		ctx:               ctx,
		logicalDatabaseId: logicalDatabaseId,
...
func (a *LogicalDatabaseApiService) ListLogicalDatabaseContainerExecute(r ApiListLogicalDatabaseContainerRequest) (*ContainerResponseList, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodGet
		localVarPostBody    interface{}
		formFiles           []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "LogicalDatabaseApiService.ListLogicalDatabaseContainer")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/logicalDatabase/{logicalDatabaseId}/container"
	localVarPath = strings.Replace(localVarPath, "{"+"logicalDatabaseId"+"}", url.PathEscape(parameterToString(r.logicalDatabaseId, "")), -1)
